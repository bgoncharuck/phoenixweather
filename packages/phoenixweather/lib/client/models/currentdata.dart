...
class CurrentData extends Equatable {
  ...
  CurrentData.fromOW(OpenWeatherModel owm)
      : location = owm.location.data,
        dt = owm.dt,
        hourly = List.from(
            owm.hourly.map((hour) => WeatherModel.fromOWHour(hour)).toList()),
        daily = List.from(
            owm.daily.map((day) => DailyWeatherModel.fromOWDay(day)).toList());
  ...
  Map<String, dynamic> toJson() =>
      {'location': location, 'dt': dt, 'hourly': hourly, 'daily': daily};

  @override
  List<Object> get props => [location, dt, hourly, daily];
  @override
