  ...
  Widget build(BuildContext context) {
    ...
    searchBloc.add(
      SearchEventLocationEdited(
          text: (database.user.home != null) ? database.user.home : ""),
    );

    return BlocBuilder<SearchBloc, SearchState>(
      builder: (BuildContext context, SearchState locationSearch) {
        if (locationSearch is SearchStateSuccess ||
            locationSearch is SearchStatePrevious) {
          if (locationSearch is SearchStateSuccess &&
              database.user.id != "local") {
...
class PhoenixWeatherShow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    showBloc.add(ShowDayEvent(data: data, day: 1, innerEvent: true));
