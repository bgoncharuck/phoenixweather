...
Future<bool> loadFiles(
    {@required RuntimeDatabase database, @required bool noInternet}) async {
  if (noInternet) await loadNetworkJson(database);
...
Future createWeather() async {
  await storage.write(data: 'init', asFile: weatherStorageFile);
}
...
Future createLocation() async {
  await storage.write(data: 'init', asFile: locationsStorageFile);
}
...
Future createLocalUser() async {
  await storage.write(data: 'init', asFile: userStorageFile);
}
