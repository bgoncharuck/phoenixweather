...
Future testAll() async {
  ...
  if (weather == null) {
    ...
    else print("Error: internal language error");
  } else print("Temperature in ${locationModel.data} is ${weather.hourly[0].temp} K");
  print("\n--------\n\n");
 
 
  // json test
  final jsonTest= json.decode(json.encode(weather.toJson()));
  assert(OpenWeatherModel.fromJson(jsonTest).location.data == weather.location.data, 
  "JSON test failed");
}